...
Decals::Decals(Engine::VulkanContext* context, Renderer* renderer, std::string textureFilename) {
...
void Decals::uploadImage(const std::string& textureFilename) {
	...
	Engine::TextureBufferSetting decalSetting = {
	...
	std::pair<Engine::vk::Texture, Engine::vk::ImageView> texture = createTextureBuffer(*this->context, decalSetting);
	...
	Engine::vk::Buffer stagingBuf = Engine::vk::createBuffer(
		"decalStaging",
		*this->context->allocator,
		sizeInBytes,
		VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
	...
	Engine::beginCommandBuffer(cmdBuf);
...
void Decals::createDescriptors() {
	this->decalUniform = Engine::vk::createBuffer(
		"decalTransformsUBO",
		*this->context->allocator,
		sizeof(glm::mat4) * 100,
