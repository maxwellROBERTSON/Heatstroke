...
namespace Utils {
    ...
    std::unordered_set<std::string> getInstanceLayers() {
		...
		if( auto const res = vkEnumerateInstanceLayerProperties( &numLayers, layers.data() ); VK_SUCCESS != res )
		{
			throw Error( "Unable to get layer properties\n"
				"vkEnumerateInstanceLayerProperties() returned %s", toString(res).c_str()
			);
		}
	...
	std::unordered_set<std::string> getInstanceExtensions()
	{
		...
		{	
			throw Error( "Unable to enumerate extensions\n"
				"vkEnumerateInstanceExtensionProperties() returned %s", toString(res).c_str() 
			);
		}
	...
	VkInstance createInstance(const std::vector<const char*>& aEnabledLayers, const std::vector<const char*>& aEnabledExtensions, bool aEnableDebugUtils) {
		...
		appInfo.pApplicationName    = "Heatstroke"; // THIS IS TEMP UNTIL WE DECIDE ON A NAME
		appInfo.applicationVersion  = 1; // I dont think it matters much if we ever increment this for the group project
		...
		if( auto const res = vkCreateInstance( &instanceInfo, nullptr, &instance ); VK_SUCCESS != res )
			throw Error( "Unable to create Vulkan instance\n vkCreateInstance() returned %s", toString(res).c_str());
    ...
    VkDebugUtilsMessengerEXT createDebugMessenger(VkInstance aInstance) {
		...
		if( auto const res = vkCreateDebugUtilsMessengerEXT( aInstance, &debugInfo, nullptr, &messenger ); VK_SUCCESS != res )
			throw Error( "Unable to set up debug messenger\n vkCreateDebugUtilsMessengerEXT() returned %s", toString(res).c_str());
	...
	VKAPI_ATTR VkBool32 VKAPI_CALL debugUtilCallback(
        ...
        const VkDebugUtilsMessengerCallbackDataEXT* aData,
        void* /*aUserPtr*/ ) {

		if( 1461184347 == aData->messageIdNumber )
			return VK_FALSE;

		std::fprintf( stderr, "%s (%s): %s (%d)\n%s\n--\n", toString(aSeverity).c_str(), messageTypeFlags(aType).c_str(), aData->pMessageIdName, aData->messageIdNumber, aData->pMessage );
    ...
    std::unordered_set<std::string> getDeviceExtensions(VkPhysicalDevice aPhysicalDev)
	{
		...
		if( auto const res = vkEnumerateDeviceExtensionProperties( aPhysicalDev, nullptr, &extensionCount, nullptr ); VK_SUCCESS != res )
		{
			throw Error( "Unable to get device extension count\n"
				"vkEnumerateDeviceExtensionProperties() returned %s", toString(res).c_str()
			);
		}
