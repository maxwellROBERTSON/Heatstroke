...
namespace Engine {
	...
	Decals::Decals(VulkanContext* context, std::string textureFilename, std::uint32_t maxDecals) {
	...
	/*
	* Sets next decal at the position aligned at the provided normal. This function
	* will automatically align the decal to the surface normal given.
	* 
	* @param position - World space position to set the next decal
	* @param normal - The surface normal of where the center of the decal will be projected
	*/
	void Decals::setNextDecal(physx::PxVec3 position, physx::PxVec3 normal) {
		...
		if (this->leadDecal == this->maxDecals)
			this->leadDecal = 0;

		this->decalTransforms[this->leadDecal] = glm::translate(glm::mat4(1.0f), glm::vec3(adjustedPos.x, adjustedPos.y, adjustedPos.z)) * glm::rotate(glm::mat4(1.0f), angle, axis) * glm::scale(glm::mat4(1.0f), glm::vec3(0.025f, 0.025f, 1.0f));
	...
	void Decals::updateUniform(VkCommandBuffer cmdBuf) {
		...
		vkCmdUpdateBuffer(cmdBuf, this->decalUniform.buffer, 0, sizeof(glm::mat4) * this->maxDecals, &this->decalTransforms);
	...
	void Decals::uploadVertices() {
		...
		this->posBuffer = vk::createBuffer(
			"decalPosBuffer",
		...
		this->indicesBuffer = vk::createBuffer(
			"decalIndicesBuffer",
			*this->context->allocator,
			indexSize,
		...
		vk::Fence uploadComplete = createFence(*this->context->window);
	...
	void Decals::createDescriptors() {
		...
		std::vector<DescriptorSetting> vertexUBOSetting = { {VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, VK_SHADER_STAGE_VERTEX_BIT} };
		this->transformDescriptorLayout = createDescriptorLayout(*this->context->window, vertexUBOSetting);

		this->transformDescriptorSet = createUBODescriptor(*this->context->window, this->transformDescriptorLayout.handle, this->decalUniform.buffer);
		...
		this->imageDescriptorLayout = createDescriptorLayout(*this->context->window, fragImageSetting);

		this->imageDescriptorSet = createImageDescriptor(
			*this->context->window,
			this->imageDescriptorLayout.handle,
			VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
