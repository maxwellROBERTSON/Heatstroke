...
#include "../third_party/AL/sndfile.h"
#include <iostream>
...
ALuint SoundBuffer::addSoundEffect(const char* filename)
{
	...
	//open the audio file and check that its usable
	...
	alBufferData(buffer, format, membuf, num_bytes, sfinfo.samplerate); //upload data to buffer

	//free up the memory that was allocated for the buffer
	free(membuf);
	sf_close(sndfile);
