...
namespace vk {
	...
	void AnimationSampler::translate(Node* node, float interpolation, std::size_t index, bool blend) {
		switch (this->interpolationType) {
		case AnimationSampler::InterpolationType::LINEAR: {
			if (blend) {
				node->translation = glm::mix(node->translation, glm::vec3(outputValues[index + 1]), interpolation);
			}
			else {
	...
	void AnimationSampler::rotate(Node* node, float interpolation, std::size_t index, bool blend) {
		switch (this->interpolationType) {
		case AnimationSampler::InterpolationType::LINEAR: {
			if (blend) {
				glm::quat q2;
				...
				node->rotation = glm::normalize(glm::slerp(node->rotation, q2, interpolation));
			}
			else {
				glm::quat q1;
	...
	void AnimationSampler::scale(Node* node, float interpolation, std::size_t index, bool blend) {
		switch (this->interpolationType) {
		case AnimationSampler::InterpolationType::LINEAR: {
			if (blend) {
				node->scale = glm::mix(node->scale, glm::vec3(outputValues[index + 1]), interpolation);
			}
			else {
