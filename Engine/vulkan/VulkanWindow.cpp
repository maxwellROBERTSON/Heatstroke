...
#		endif // ~ debug builds
	...
	void recreateFormatDependents(const VulkanWindow& aWindow, std::map<std::string, vk::RenderPass>& aRenderPasses) {
		...
		aRenderPasses["default"] = createRenderPass(aWindow);
		aRenderPasses["deferred"] = createDeferredRenderPass(aWindow);
	}
		...
		std::map<std::string, vk::Pipeline>& aPipelines)
	{
		...
		aBuffers["normalsBuffer"] = createNormalsBuffer(*aContext.window, *aContext.allocator);
		aBuffers["albedoBuffer"] = createAlbedoBuffer(*aContext.window, *aContext.allocator);
		aBuffers["emissiveBuffer"] = createEmissiveBuffer(*aContext.window, *aContext.allocator);

		aPipelines["default"] = createPipeline(*aContext.window, aRenderPasses["default"].handle, aPipelineLayouts["default"].handle);
		std::tuple<vk::Pipeline, vk::Pipeline> deferredPipelines = createDeferredPipelines(*aContext.window, aRenderPasses["deferred"].handle, aPipelineLayouts["default"].handle, aPipelineLayouts["deferred"].handle);
		aPipelines["gBufWrite"] = std::move(std::get<0>(deferredPipelines));
		aPipelines["deferred"] = std::move(std::get<1>(deferredPipelines));
		...
		std::map<std::string, VkDescriptorSet>& aDescriptorSets)
	{
		...
		aDeferredFramebuffers.clear();
		Engine::createDeferredFramebuffers(aWindow, aDeferredFramebuffers, aRenderPasses["deferred"].handle, std::get<1>(buffers["depthBuffer"]).handle, std::get<1>(buffers["normalsBuffer"]).handle, std::get<1>(buffers["albedoBuffer"]).handle, std::get<1>(buffers["emissiveBuffer"]).handle);
	
		aDescriptorSets["deferredShadingDescriptor"] = Engine::createDeferredShadingDescriptor(aWindow, aDescriptorLayouts["deferredLayout"].handle, std::get<1>(buffers["depthBuffer"]).handle, std::get<1>(buffers["normalsBuffer"]).handle, std::get<1>(buffers["albedoBuffer"]).handle, std::get<1>(buffers["emissiveBuffer"]).handle);
	}
